...
class DragAndDropZip extends StatefulWidget {
  const DragAndDropZip({super.key, required this.beatId});

  final String beatId;

  @override
  State<DragAndDropZip> createState() => _DragAndDropZipState();
}

class _DragAndDropZipState extends State<DragAndDropZip> {
      ...
      buildWhen: (previous, current) {
        if (previous is BeatEditState && current is BeatEditState) {
          return previous.beat != current.beat ||
              previous.isZipLoading != current.isZipLoading ||
              previous.progressZip != current.progressZip;
        }
        return false;
      },
        ...
        if (state is BeatEditState) {
          return Stack(
            children: [
              Container(
                ...
                child: Stack(
                  children: [
                    ...
                    if (state.isZipLoading == IsZipLoading.loading)
                      Center(
                        child: CircularProgressIndicator(
                          color: Colors.white,
                          value: state.progressZip,
                          strokeWidth: 3,
                        ),
                      ),
                    if (state.isZipLoading == IsZipLoading.success ||
                        state.beat.availableFiles.zipUrl != "")
                      const Center(
                        child: Text(
                          "Файл загружен",
                          style: TextStyle(
              ...
              if (state.beat.availableFiles.zipUrl == "" &&
                  state.isZipLoading != IsZipLoading.loading)
                Positioned(
                  left: 0,
                  right: 0,
                  top: 0,
