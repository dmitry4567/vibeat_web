...
class DragAndDropMp3 extends StatefulWidget {
  const DragAndDropMp3({super.key, required this.beatId});

  final String beatId;

  @override
  State<DragAndDropMp3> createState() => _DragAndDropMp3State();
}

class _DragAndDropMp3State extends State<DragAndDropMp3> {
      ...
      buildWhen: (previous, current) {
        if (previous is BeatEditState && current is BeatEditState) {
          return previous.beat != current.beat ||
              previous.isMp3Loading != current.isMp3Loading ||
              previous.progressMp3 != current.progressMp3;
        }
        return false;
      },
        ...
        if (state is BeatEditState) {
          return Stack(
            children: [
              Container(
                ...
                child: Stack(
                  children: [
                    ...
                    if (state.isMp3Loading == IsMp3Loading.loading)
                      Center(
                        child: CircularProgressIndicator(
                          color: Colors.white,
                          value: state.progressMp3,
                          strokeWidth: 3,
                        ),
                      ),
                    if (state.isMp3Loading == IsMp3Loading.success ||
                        state.beat.availableFiles.mp3Url != "")
                      const Center(
                        child: Text(
                          "Файл загружен",
                          style: TextStyle(
              ...
              if (state.beat.availableFiles.mp3Url == "" &&
                  state.isMp3Loading != IsMp3Loading.loading)
                Positioned(
                  left: 0,
                  right: 0,
                  top: 0,
